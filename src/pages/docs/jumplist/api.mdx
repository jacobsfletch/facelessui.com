...
This component provides the [jumplist context](#context) to your app. It does not have any required props and renders nothing in the DOM. This is where the [global settings](#provider-props) are defined.
...
Will inject `html: { scroll-behavior: smooth; }` as inline CSS onto the root html element of your DOM. See [smooth-scrolling](`${process.env.NEXT_PUBLIC_APP_URL}/docs/jumplist/setup#smooth-scroll`) for more details. Defaults to `true`.
...
This is an array of jumplist nodes, each with its [isIntersecting](#isIntersecting) status.
...
The `type` attribute is automatically set to "button" when the [htmlElement](#button-htmlElement) is `button`.
...
A hook used to access the [jumplist context](#context).
