...
Latest version: <VersionNumber name="jumplist" />

## Installation

<InstallationCode name="jumplist" />

To track elements as they enter and exit the viewport, you first need to wrap your app with the [`<JumplistProvider>`](/docs/jumplist/api#provider). This does not render anything in the DOM and is where the [global settings](/docs/jumplist/api#provider-props) are defined. This will maintain the state of every jumplist node in your app and provide this state through context.
...
Then, use a [`<JumplistNode>`](/docs/jumplist/api#node) for every element you want to track. The only required prop of this component is the `nodeID`, the unique string identifying each node.
...
To navigate jumplist nodes we rely entirely on native HTML behavior. By adding a hash to the URL that matches the id of an element in the document, the browser will automatically scroll to that element. You can also enable [smooth scrolling](#/docs/jumplist/setup#smooth-scroll).
...
Or use the [smoothScroll](/docs/jumplist/api#provider-props) prop on the provider, which sets this property as inline CSS.
