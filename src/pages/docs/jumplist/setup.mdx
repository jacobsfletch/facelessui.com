...
Then, use a [`<JumplistNode>`](/docs/jumplist/api#jumplistnode) for every element you want to track. The only required prop of this component is the `nodeID`, the unique string identifying each node.
...
Or use the [smoothScroll](/docs/jumplist/api#jumplistprovider-props) prop on the provider, which sets this property as inline CSS.
