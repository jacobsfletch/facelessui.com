...
A unique ID used to match ARIA attributes between the slider components. Defaults to a random 5 character string. See [accessibility](#accessibility) for full details.
...
NOTE: To match ARIA attributes between DOM elements, a random 5 character string is generated as the ID. To opt out of this, just pass your own [`id`](#id) to the `<SliderProvider>` and it will be used instead.
