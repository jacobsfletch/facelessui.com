...
Latest version: <VersionNumber  name="slider" />

## Installation

<InstallationCode name="slider" />

To create a slider, wrap any portion of your app with the [`<SliderProvider>`](/docs/slider/api#provider). This component does not render anything in the DOM and is where the [slider settings](/docs/slider/api#provider-props) are defined. Inside of this, render a [`<SliderTrack>`](/docs/slider/api#track). This will add a scrollable element onto the page which overflows its content as necessary. Then, nest as many [`<Slide>`](/docs/slider/api#slide) components within the track as necessary.
...
If your slider is not free-scrolling, drag-scrolling, or auto-playing, you will need to render navigation buttons. Conveniently, the [`<SliderButton>`](/docs/slider/api#button) component solves the most common navigation needs and is highly customizable.
...
Alternatively, you could wire up your own button using methods on the [slider context](/docs/slider/api#context). To do this, access the `goToNextSlide` method with the `useSlider` hook.
